...
- [x] Add assertions that the dashboard script defines state save/restore helpers.
- [x] Assert manual refresh and HTMX lifecycle events call the helpers.
- [x] Assert the initial page boot restores state before applying filter and selection.
...
- [x] Add `sessionStorage` helpers with fail-soft `try/catch`.
- [x] Persist state from surface switches, filter input, review row selection, and before HTMX requests.
- [x] Restore state on boot and after HTMX swaps.
- [x] Keep dashboard quit behavior local to the current page session.
...
- [x] Mark WUI state-preserving refresh as resolved.
- [x] Keep remaining findings focused on identity panel, mobile table layout, table contract, and action affordances.
