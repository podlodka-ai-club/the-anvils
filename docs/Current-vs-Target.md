...
- **Implemented:** deterministic task state, Postgres queueing, plan import,
  ...
  stage lifecycle, and configured verification commands that block `DONE` on
  required failure.
- **Partial:** project profiles, built-in profile vocabulary, human-review
  approval/enforcement, PR creation as a configured sink, multi-repo execution,
  and dashboard/operator projections.
